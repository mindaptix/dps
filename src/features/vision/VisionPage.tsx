...
const futureSignals = [
  { label: 'Artificial intelligence', detail: 'Transforming industries', icon: 'ai', image: '/images/child-journey-grade12-ai.png' },
  { label: 'New career paths', detail: 'Emerging every day', icon: 'career', image: '/images/leadership.png' },
  { label: 'Connected world', detail: 'Reshaping how we live and learn', icon: 'connected', image: '/images/learning-that-matters.png' },
] as const

const beliefMoments = [
  { title: 'Seen deeply', text: 'Children build confidence when adults notice who they are becoming.', image: '/images/vision/we-see-children.png' },
  { title: 'Safe to try', text: 'Questions, mistakes, and new ideas need classrooms that feel safe.', image: '/images/why-we-exist-question-lab.png' },
  { title: 'Known by name', text: 'Belonging is not a slogan. It is built through daily relationships.', image: '/images/together-with-families.png' },
] as const
...
function FutureSignalIcon({ type }: { type: (typeof futureSignals)[number]['icon'] }) {
  ...
  return (
    <svg viewBox="0 0 48 48" aria-hidden="true" className="h-7 w-7" fill="none">
      ...
      <circle cx="36" cy="14" r="3.5" fill="#fff8eb" stroke="currentColor" strokeWidth="1.5" />
    </svg>
  )
}
...
function Kicker({ children, light = false }: { children: React.ReactNode; light?: boolean }) {
  return (
    <p className={`flex items-center gap-4 text-[0.72rem] font-bold uppercase tracking-[0.28em] sm:text-[0.82rem] ${light ? 'text-[#a8cda0]' : 'text-[#6c9469]'}`}>
      <span className={`h-px w-10 ${light ? 'bg-[#88b17f]' : 'bg-[#79a273]'}`} />{children}
    </p>
  )
}

function cardDetail(text: string) {
  const normalized = text.toLowerCase()
  if (normalized.includes('academic')) return 'Strong foundations, disciplined learning, and joyful mastery.'
  if (normalized.includes('character')) return 'Integrity, kindness, responsibility, and inner strength.'
  if (normalized.includes('wellbeing')) return 'A calm, caring environment where children feel secure.'
  if (normalized.includes('future')) return 'Skills and mindsets for a changing world.'
  if (normalized.includes('creativity')) return 'Space to imagine, make, question, and express.'
  if (normalized.includes('leadership')) return 'Confidence to take initiative and serve others.'
  if (normalized.includes('human values')) return 'Empathy and wisdom at the centre of growth.'
  if (normalized.includes('happy')) return 'A school day children can look forward to.'
  if (normalized.includes('confidence')) return 'Small wins that become lasting self-belief.'
  if (normalized.includes('strengths')) return 'Every child is helped to discover what they carry within.'
  if (normalized.includes('good human')) return 'Learning shaped by kindness and responsibility.'
  if (normalized.includes('known') || normalized.includes('valued')) return 'Seen as an individual, not just as a student.'
  if (normalized.includes('challenged')) return 'Stretched with care, not pressure.'
  if (normalized.includes('supported')) return 'Guidance that stays close when learning gets hard.'
  if (normalized.includes('inspired')) return 'Experiences that open possibility.'
  if (normalized.includes('classrooms')) return 'Designed around attention, curiosity, and participation.'
  if (normalized.includes('relationships')) return 'Trust between children, teachers, and families.'
  if (normalized.includes('campus')) return 'Spaces that invite movement, discovery, and belonging.'
  if (normalized.includes('culture')) return 'Daily habits that make values visible.'
  if (normalized.includes('opportunities')) return 'More ways for children to find their voice.'
  if (normalized.includes('expectations')) return 'A clear standard held with warmth.'
  return 'A meaningful part of the education we are building.'
}

function NumberCard({ text, index, dark = false }: { text: string; index: number; dark?: boolean }) {
  const reduceMotion = useReducedMotion()
  const detail = cardDetail(text)
  ...
  return (
    <motion.article
      ...
      className={`vision-number-card group relative min-h-72 overflow-hidden rounded-[1.6rem] border p-6 shadow-[0_28px_80px_rgba(25,61,40,0.1)] ${dark ? 'border-white/10 bg-white/[0.07] text-white backdrop-blur' : 'border-[#d7e5d3] bg-white text-[#224333]'}`}
    >
      <div className="vision-number-card-glow pointer-events-none absolute inset-0" />
      <span className={`absolute -right-3 -top-8 text-[6.5rem] font-semibold tracking-[-0.08em] transition duration-500 ${dark ? 'text-white/[0.05] group-hover:text-white/[0.1]' : 'text-[#eff5ed] group-hover:text-[#dfeadd]'}`}>0{index + 1}</span>
      <div className="relative flex items-start justify-between gap-4">
        <span className={`rounded-full border px-3 py-1.5 text-[0.58rem] font-bold tracking-[0.2em] ${dark ? 'border-white/15 bg-white/10 text-[#a5c89e]' : 'border-[#d7e5d3] bg-[#f6faf3] text-[#72966f]'}`}>0{index + 1}</span>
        <span className={`vision-number-card-icon flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border ${dark ? 'border-white/15 bg-white/10 text-[#d7ebd2]' : 'border-[#e1cfaa] bg-[#fff8eb] text-[#9b6b22]'}`}>
          <svg viewBox="0 0 48 48" aria-hidden="true" className="h-7 w-7" fill="none">
            {iconType === 0 && <path d="M12 33V16h24v17M16 20h16M16 25h11M19 38h10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
            {iconType === 1 && <path d="M24 9 29 19l11 1.5-8 7.8 1.9 11-9.9-5.2-9.9 5.2 1.9-11-8-7.8L19 19l5-10Z" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" />}
            {iconType === 2 && <path d="M14 25c2.4 6 5.8 9 10 9s7.6-3 10-9M17 18h.1M31 18h.1M24 8v4m0 24v4" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />}
            {iconType === 3 && <path d="M12 35 22 25l6 6 9-16M30 15h7v7M12 15h8M12 21h5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
          </svg>
        </span>
      </div>
      <div className={`vision-number-card-visual relative mt-7 overflow-hidden rounded-[1.2rem] border ${dark ? 'border-white/10 bg-white/[0.06]' : 'border-[#dce8d8] bg-[#f4f8f1]'}`}>
        <span className="absolute inset-0 bg-[radial-gradient(circle_at_28%_22%,rgba(216,188,101,0.34),transparent_32%),linear-gradient(135deg,rgba(111,148,106,0.18),transparent_58%)]" />
        <span className={`absolute bottom-4 left-4 h-16 w-16 rounded-full border ${dark ? 'border-white/20 bg-white/10' : 'border-[#cbdcc7] bg-white/70'}`} />
        <span className={`absolute right-5 top-5 h-3 w-3 rounded-full ${dark ? 'bg-[#d8bc65]' : 'bg-[#7aa174]'} shadow-[0_0_18px_currentColor]`} />
      </div>
      <p className="relative mt-6 text-xl leading-7 tracking-[-0.02em]">{text}</p>
      <p className={`relative mt-3 text-sm leading-6 ${dark ? 'text-white/58' : 'text-[#60796a]'}`}>{detail}</p>
    </motion.article>
  )
}
...
export function VisionPage() {
  ...
  return (
    <main className="overflow-x-clip bg-[#f7f1e6] text-[#17233a]">
      <motion.div className="fixed inset-x-0 top-0 z-[80] h-[3px] origin-left bg-gradient-to-r from-[#d8bc65] via-[#b9862f] to-[#17233a]" style={{ scaleX: progressScale }} />

      <section ref={heroRef} className="relative min-h-[100svh] overflow-hidden bg-[#f6efe1]">
        ...
        <div className="vision-orb absolute -left-36 top-[34%] h-[28rem] w-[28rem] rounded-full border border-[#b9862f]/25" />
        ...
        <motion.div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[112rem] items-center px-6 pb-24 pt-40 sm:px-10 lg:px-16 xl:px-24" style={reduceMotion ? undefined : { y: heroCopyY, opacity: heroOpacity }}>
          <div className="max-w-[66rem]">
            ...
            <motion.h1 initial={reduceMotion ? false : { opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.25, ease: [0.22, 1, 0.36, 1] }} className="vision-hero-title mt-9 font-semibold uppercase text-[#0d321f]">
              Why <span className="block">another</span><span className="vision-hero-school block font-serif font-normal italic normal-case text-[#6c9d68]">school?</span>
            </motion.h1>
            <motion.p initial={reduceMotion ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }} className="mt-12 max-w-3xl text-2xl leading-10 text-[#3f5c4c] sm:text-[1.65rem] sm:leading-[2.9rem]">
              The world our children are growing up in is changing faster than at any other time in history.
            </motion.p>
          </div>
        </motion.div>
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.85 }} className="absolute bottom-7 right-6 z-20 hidden w-[27rem] rounded-[1.5rem] border border-white/25 bg-[#17233a]/80 p-8 text-white shadow-[0_25px_80px_rgba(12,20,36,0.28)] backdrop-blur-xl md:block lg:right-16 xl:right-24">
          <p className="font-serif text-3xl italic leading-10">But because children deserve an education designed for the life ahead.</p>
        </motion.div>
      </section>

      <section className="vision-future-section relative overflow-hidden px-6 py-20 sm:px-10 lg:px-16 lg:py-24 xl:px-24">
        <div className="vision-future-glow pointer-events-none absolute -right-40 top-4 h-[36rem] w-[36rem] rounded-full" />
        <div className="vision-future-dots pointer-events-none absolute inset-0 opacity-45" />
        <div className="relative mx-auto grid max-w-[96rem] gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:gap-16">
          <Reveal className="relative">
            <Kicker>WHY ANOTHER SCHOOL?</Kicker>
            <h2 className="mt-6 max-w-lg text-4xl font-medium leading-[1.02] tracking-[-0.05em] sm:text-6xl">The world is changing faster than ever.</h2>
            <div className="mt-6 grid max-w-lg grid-cols-[1.1fr_0.9fr] gap-3">
              <motion.div
                animate={reduceMotion ? undefined : { y: [0, -10, 0] }}
                transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
                  className="vision-future-left-image group relative overflow-hidden rounded-[1.4rem] border border-white/70 shadow-[0_28px_80px_rgba(35,70,49,0.16)]"
              >
                <Image src="/images/why-we-exist-question-lab.png" alt="Students exploring questions in a learning space" width={720} height={860} sizes="(max-width:1024px) 60vw, 26vw" className="h-full w-full object-cover transition-transform duration-[1400ms] group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0f2f21]/45 via-transparent to-transparent" />
              </motion.div>
              <div className="grid gap-3">
                <motion.div
                  animate={reduceMotion ? undefined : { y: [0, 8, 0] }}
                  transition={{ duration: 5.5, repeat: Infinity, ease: 'easeInOut' }}
                  className="vision-future-small-image group relative overflow-hidden rounded-[1.1rem] border border-white/70 shadow-[0_18px_55px_rgba(35,70,49,0.12)]"
                >
                  <Image src="/images/learning-that-matters.png" alt="Children learning through meaningful activity" width={520} height={420} sizes="(max-width:1024px) 36vw, 14vw" className="h-full w-full object-cover transition-transform duration-[1400ms] group-hover:scale-105" />
                </motion.div>
                <motion.div
                  ...
                  className="vision-future-small-image group relative overflow-hidden rounded-[1.1rem] border border-white/70 shadow-[0_18px_55px_rgba(35,70,49,0.12)]"
                >
                  <Image src="/images/child-journey-grade12-ai.png" alt="A future-ready student learning with technology" width={520} height={420} sizes="(max-width:1024px) 36vw, 14vw" className="h-full w-full object-cover transition-transform duration-[1400ms] group-hover:scale-105" />
                </motion.div>
              </div>
            </div>
            <p className="mt-5 max-w-md text-base leading-7 text-[#5a7264]">Education has to feel alive, visual, and ready for the life children are stepping into.</p>
            <div className="mt-5 grid max-w-md grid-cols-3 gap-2">
              {['AI', 'Careers', 'Life'].map((item, index) => (
                <motion.span
                  key={item}
                  animate={reduceMotion ? undefined : { y: [0, index % 2 ? 8 : -8, 0] }}
                  transition={{ duration: 4 + index, repeat: Infinity, ease: 'easeInOut' }}
                  className="rounded-full border border-[#d8bc65]/45 bg-white/60 px-4 py-3 text-center text-[0.62rem] font-bold uppercase tracking-[0.22em] text-[#6f946a] shadow-[0_18px_45px_rgba(83,104,76,0.08)] backdrop-blur"
                >
                  {item}
                </motion.span>
              ))}
            </div>
            <div className="mt-5 flex items-center gap-3 text-[0.62rem] font-bold uppercase tracking-[0.22em] text-[#769572]">
              <span className="relative flex h-3 w-3"><span className="absolute inset-0 animate-ping rounded-full bg-[#b9862f]/35" /><span className="relative m-auto h-1.5 w-1.5 rounded-full bg-[#b9862f]" /></span>
              The world is already moving
            </div>
          </Reveal>
          <Reveal delay={0.08}>
            <div className="vision-future-image-panel group relative mb-4 overflow-hidden rounded-[1.6rem] border border-[#d5e3d1] bg-[#173b2a] shadow-[0_35px_100px_rgba(35,70,49,0.18)]">
              <Image src="/images/future-still-being-written.png" alt="Children learning with future-ready tools" width={1180} height={620} sizes="(max-width:1024px) 100vw, 58vw" className="h-full w-full object-cover opacity-85 transition-transform duration-[1600ms] group-hover:scale-[1.045]" />
              <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(18,50,35,0.78)_0%,rgba(18,50,35,0.28)_52%,rgba(18,50,35,0.1)_100%)]" />
              <div className="vision-image-sweep absolute inset-y-0 -left-1/2 w-1/3 skew-x-[-18deg] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
              <div className="absolute bottom-6 left-6 max-w-sm text-white">
                <p className="text-[0.62rem] font-bold uppercase tracking-[0.24em] text-[#c8e1c1]">Future signals</p>
                <p className="mt-2 font-serif text-2xl italic leading-tight">Learning must meet the world children are entering.</p>
              </div>
            </div>
            <div className="vision-signal-map relative space-y-3">
              {futureSignals.map((signal, index) => (
                <motion.article
                  ...
                  whileHover={reduceMotion ? undefined : { x: 14, scale: 1.018 }}
                  viewport={{ once: true, amount: 0.4 }}
                  transition={{ duration: 0.55, delay: index * 0.08, ease: [0.22, 1, 0.36, 1] }}
                  className="vision-signal-card group relative flex items-center gap-4 overflow-hidden rounded-[1.2rem] border border-[#d5e3d1] bg-white/85 p-3 shadow-[0_22px_70px_rgba(37,74,49,0.1)] backdrop-blur-md sm:p-4"
                >
                  <span className="vision-signal-thumb relative hidden shrink-0 overflow-hidden rounded-[1.1rem] border border-white/70 shadow-[0_18px_45px_rgba(35,70,49,0.16)] md:block">
                    <Image src={signal.image} alt="" width={180} height={140} sizes="120px" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  </span>
                  <span className="vision-signal-icon flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[#e1cfaa] bg-[#fff8eb] text-[#9b6b22] transition-colors duration-300 group-hover:bg-[#17233a] group-hover:text-white">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-lg font-medium tracking-[-0.025em] text-[#234533] sm:text-xl">{signal.label}</span>
            ...
            <p className="mt-5 max-w-2xl border-l border-[#86aa7f] pl-6 text-base leading-7 text-[#5a7264]">Yet parents are often left with the same questions:</p>
          </Reveal>
        </div>
        <Reveal className="vision-parent-panel relative mx-auto mt-14 grid max-w-[96rem] overflow-hidden rounded-[2rem] border border-[#d6e3d2] bg-white/70 shadow-[0_35px_100px_rgba(35,70,49,0.12)] backdrop-blur lg:grid-cols-[0.92fr_1.08fr]">
          <div className="vision-parent-panel-image relative overflow-hidden">
            <Image src="/images/together-with-families.png" alt="Parents, teachers, and children learning together" width={900} height={720} sizes="(max-width:1024px) 100vw, 44vw" className="h-full w-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#102f21]/65 via-transparent to-transparent" />
            <div className="absolute bottom-6 left-6 right-6 text-white">
              <p className="text-[0.62rem] font-bold uppercase tracking-[0.24em] text-[#d5ebd0]">Parent concerns</p>
              <p className="mt-3 font-serif text-3xl italic leading-tight">The right school answers what families quietly ask.</p>
            </div>
          </div>
          <div className="p-6 sm:p-8 lg:p-10">
            <Kicker>THE QUESTIONS THAT MATTER</Kicker>
            <h3 className="mt-5 max-w-2xl text-3xl font-medium leading-tight tracking-[-0.04em] text-[#173b2a] sm:text-4xl">Not more decoration. A clearer promise to parents.</h3>
            <div className="mt-7 grid gap-3">
              {parentQuestions.map((question, index) => (
                <motion.div
                  ...
                  className="group flex items-center gap-4 rounded-[1.1rem] border border-[#d7e4d3] bg-[#f8fbf6]/85 p-3 text-[#234533] shadow-[0_14px_40px_rgba(35,70,49,0.06)]"
                >
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#173b2a] text-[#d5ebd0] transition-colors duration-300 group-hover:bg-[#9b6b22]">
                    <span className="scale-75"><QuestionSymbol index={index} /></span>
                  </span>
                  <span className="text-base leading-6 sm:text-lg">{question}</span>
                </motion.div>
              ))}
            </div>
          </div>
        </Reveal>
        <Reveal className="relative mx-auto mt-20 max-w-5xl rounded-[2.2rem] bg-[#173b2a] p-8 text-center text-white shadow-[0_40px_120px_rgba(22,59,42,0.2)] sm:p-12 lg:p-16">
          ...
          <div className="relative">
            ...
            <h2 className="mx-auto mt-6 max-w-4xl text-3xl font-light leading-tight tracking-[-0.04em] sm:text-5xl">Is school preparing children for the world they will actually inherit?</h2>
            <div className="mx-auto my-8 h-px max-w-sm bg-gradient-to-r from-transparent via-[#91ba88] to-transparent" />
        ...
        <div className="vision-section-endcap pointer-events-none absolute inset-x-0 bottom-0 h-24" />
      </section>

      <section className="vision-belief-section relative overflow-hidden bg-[#17233a] px-6 py-28 text-white sm:px-10 lg:px-16 lg:py-44 xl:px-24">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#d8bc65]/70 to-transparent" />
        <div className="absolute left-1/2 top-0 hidden -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#d8bc65]/35 bg-[#17233a] px-5 py-2 text-[0.58rem] font-bold uppercase tracking-[0.22em] text-[#d8bc65] shadow-[0_18px_55px_rgba(0,0,0,0.25)] lg:block">Chapter 03</div>
        <div className="vision-grid absolute inset-0 opacity-35" />
        <div className="vision-belief-ambient pointer-events-none absolute inset-0" />
        <div className="relative mx-auto grid max-w-[96rem] items-center gap-16 lg:grid-cols-[1.05fr_0.95fr] lg:gap-28">
          <Reveal className="relative">
            ...
            <div className="group relative aspect-[4/4.5] overflow-hidden rounded-[2rem] shadow-[0_50px_130px_rgba(0,0,0,0.35)]">
              ...
              <div className="absolute right-6 top-6 rounded-[1.2rem] border border-white/20 bg-white/15 p-4 text-sm text-white shadow-[0_22px_60px_rgba(0,0,0,0.2)] backdrop-blur">
                <span className="block text-[0.58rem] font-bold uppercase tracking-[0.22em] text-[#c8e1c1]">Daily signal</span>
                <span className="mt-2 block font-serif text-2xl italic">Seen. Safe. Known.</span>
              </div>
            </div>
          </Reveal>
          <div>
            <Reveal><Kicker light>WHAT WE BELIEVE</Kicker><h2 className="mt-9 max-w-2xl text-5xl font-light leading-[1.02] tracking-[-0.055em] sm:text-6xl">Children grow when learning feels personal.</h2></Reveal>
            <Reveal delay={0.1} className="mt-10 grid gap-4">
              {beliefMoments.map((item, index) => (
                <motion.article
                  key={item.title}
                  whileHover={reduceMotion ? undefined : { x: 12, scale: 1.018 }}
                  transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
                  className="vision-belief-card group grid overflow-hidden rounded-[1.4rem] border border-white/12 bg-white/[0.07] shadow-[0_24px_70px_rgba(0,0,0,0.16)] backdrop-blur md:grid-cols-[11rem_1fr]"
                >
                  <div className="vision-belief-card-image relative overflow-hidden">
                    <Image src={item.image} alt="" width={320} height={220} sizes="180px" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0b2618]/35 to-transparent" />
                  </div>
                  <div className="p-5">
                    <span className="text-[0.58rem] font-bold uppercase tracking-[0.22em] text-[#a6c99e]">Belief 0{index + 1}</span>
                    <h3 className="mt-2 text-2xl font-medium tracking-[-0.035em] text-white">{item.title}</h3>
                    <p className="mt-2 text-sm leading-6 text-white/65">{item.text}</p>
                  </div>
                </motion.article>
              ))}
            </Reveal>
            <Reveal delay={0.18} className="mt-8 flex flex-wrap gap-3">{['Feel safe', 'Feel seen', 'Feel challenged', 'Know they belong'].map((item) => <motion.span whileHover={reduceMotion ? undefined : { y: -5 }} key={item} className="rounded-full border border-white/15 bg-white/[0.08] px-5 py-3 text-sm text-white/75 backdrop-blur">{item}</motion.span>)}</Reveal>
          </div>
        </div>
      </section>

      <section className="relative overflow-hidden bg-[#f7f1e6] py-28 lg:py-44">
        ...
        <div className="mx-auto mt-16 grid max-w-[96rem] gap-6 px-6 sm:px-10 lg:grid-cols-2 lg:px-16 xl:px-24">
          ...
          <Reveal delay={0.08} className="rounded-[1.8rem] bg-[#17233a] p-9 text-white shadow-[0_35px_90px_rgba(23,35,58,0.22)] sm:p-12"><p className="text-2xl leading-9 text-white/75">But in the years ahead, character may matter even more.</p><p className="mt-14 text-4xl font-light tracking-[-0.04em]">Our responsibility is not simply to prepare children for examinations.</p><p className="mt-4 font-serif text-3xl italic text-[#f5d28a]">It is to prepare them for life.</p></Reveal>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-[#17233a] px-6 py-28 text-white sm:px-10 lg:px-16 lg:py-44 xl:px-24">
      ...
      <section className="bg-[#f7f1e6] px-6 py-28 sm:px-10 lg:px-16 lg:py-44 xl:px-24"><div className="mx-auto max-w-[96rem]"><Reveal className="text-center"><Kicker>OUR COMMITMENT TO FAMILIES</Kicker><h2 className="mt-8 text-6xl font-medium tracking-[-0.055em]">A Promise We Take Seriously</h2><p className="mt-8 text-2xl text-[#4b5872]">When families choose a school, they place something precious in its care.</p><div className="mt-7 flex justify-center gap-4 font-serif text-5xl italic text-[#9b6b22]"><span>Trust.</span><span>Hope.</span><span>Possibility.</span></div><p className="mt-8 text-xl">We do not take that responsibility lightly.</p><p className="mt-3 text-xl">Our commitment is to create a school where children are:</p></Reveal><div className="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{['Known.', 'Valued.', 'Challenged.', 'Supported.', 'Inspired.', 'And encouraged to become the very best version of themselves.'].map((item, index) => <NumberCard key={item} text={item} index={index} />)}</div></div></section>
